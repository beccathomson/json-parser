...
const App = (): JSX.Element => {
  ...
  useEffect(() => {
    const treeOutput = parseTree(fileInput);
    if (treeOutput) {
  ...
  useEffect(() => {
    if (isValidJson) {
      const treeOutput = parseTreeFromJson(jsonText.trim());
      if (treeOutput) {
        setBinaryTree(treeOutput);
      }
    }
  ...
  return (
    <div>
      ...
      <div className="outputArea">
        {fileInput && (
          <pre className={isValidJson ? 'valid' : 'invalid'} contentEditable onInput={handleUserInput}>
            <code>{editableOutput}</code>
          </pre>
        )}
      </div>
